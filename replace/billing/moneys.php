...
aut(); 
only_reg();
$bill = mysql_fetch_assoc(mysql_query("select `money` from `billing` "));



if (isset($_GET['ok']) && isset($_POST['ok'])){ 

$bls = intval($_POST['wmr']*$bill['money']);
...
mysql_query("UPDATE `user` SET `money` = '".($user['money']+$bls)."' WHERE `id` = '$user[id]' LIMIT 1");

mysql_query("UPDATE `user` SET `billing` = '".($user['billing']-intval($_POST['wmr']))."' WHERE `id` = '$user[id]' LIMIT 1");

mysql_query("INSERT INTO `billing_operations` (`id_user`, `wmr`, `op`, `time`, `comm`) VALUES ('".$user['id']."', '".$wmr."', 'money', '".$time."', '".$bls."')");

$_SESSION['message'] = "Вы успешно купили $bls монет за $wmr wmr!";
...
<br /> </div>';echo "<div class='mess'>";
echo "<form method='post' action='?nick&ok'>";
if ($bill['money'] == 1) $a = 'a';
else $a = '';
echo '<font color="green">1</font> <font color="#9966cc"><b>wmr</b></font> = '.$bill['money'].' монет'.$a.'<hr>'; 
